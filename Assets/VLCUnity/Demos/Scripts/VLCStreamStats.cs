...
namespace LibVLCSharp
{
    ...
    public class VLCStreamStats : MonoBehaviour
    {
        ...
        {
            long currentMicroseconds = vlcPlayer.Time;
            long durationMicroseconds = vlcPlayer.Duration;

            // LibVLC 4 exposes time values in microseconds. Live streams usually
            // have no finite duration and may temporarily report an invalid time.
            if (durationMicroseconds <= 0)
            {
                timeText.text = _coreState == VLCState.Playing ? "LIVE" : "--:--";
                return;
            }

            if (!TryFromMicroseconds(currentMicroseconds, out TimeSpan currentTime) ||
                !TryFromMicroseconds(durationMicroseconds, out TimeSpan totalTime))
            {
                timeText.text = "--:-- / --:--";
                return;
            }

            string format = durationMicroseconds >= 3_600_000_000L ? @"hh\:mm\:ss" : @"mm\:ss";
            timeText.text = $"{currentTime.ToString(format)} / {totalTime.ToString(format)}";
        }

        private static bool TryFromMicroseconds(long microseconds, out TimeSpan result)
        {
            const long ticksPerMicrosecond = TimeSpan.TicksPerMillisecond / 1000;

            if (microseconds < 0 || microseconds > TimeSpan.MaxValue.Ticks / ticksPerMicrosecond)
            {
                result = default;
                return false;
            }

            result = TimeSpan.FromTicks(microseconds * ticksPerMicrosecond);
            return true;
        }
