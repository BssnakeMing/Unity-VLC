...
namespace LibVLCSharp
{
    ...
    public class VLCMediaPlayer : VLCVideoProviderBase
    {
        ...
        [Tooltip("Forces RTSP/RTP data over the RTSP TCP connection on Android. This avoids UDP/NAT issues on devices and emulators.")]
        public bool forceRtspOverTcpOnAndroid = true;
        ...
        private const string AndroidColorConversionShaderResource = "VLCAndroidSRGBToLinear";
        private static readonly int FlipXPropertyId = Shader.PropertyToID("_FlipX");
        private static readonly int FlipYPropertyId = Shader.PropertyToID("_FlipY");
        private Material _androidColorConversionMaterial;
        private bool _androidColorConversionShaderErrorLogged;
#endif
...
#if UNITY_ANDROID && !UNITY_EDITOR
            {
                Debug.Log($"VLCMediaPlayer Android direct open: {mediaPath}");
                Open(mediaPath, mediaOptions);
            }
#else
...
#if UNITY_ANDROID && !UNITY_EDITOR
            if (_androidColorConversionMaterial != null)
            {
                Destroy(_androidColorConversionMaterial);
                _androidColorConversionMaterial = null;
            }
#endif
        }
        #endregion
        ...
        public void Open(string path = null, params string[] options)
        {
            ...
            var finalOptions = GetEffectiveMediaOptions(trimmedPath, options);
        ...
        public async Task OpenAsync(string path = null, params string[] options)
        {
            ...
            var finalOptions = GetEffectiveMediaOptions(mediaPath, options);
        ...
        {
            var finalOptions = GetEffectiveMediaOptions(path, options);
        ...
        private void BlitToOutput(Vector2 flip)
        {
#if UNITY_ANDROID && !UNITY_EDITOR
            if (QualitySettings.activeColorSpace == UnityEngine.ColorSpace.Linear)
            {
                var material = GetAndroidColorConversionMaterial();
                if (material != null)
                {
                    material.SetFloat(FlipXPropertyId, flip.x < 0f ? 1f : 0f);
                    material.SetFloat(FlipYPropertyId, flip.y < 0f ? 1f : 0f);
                    Graphics.Blit(_vlcTexture, OutputTexture, material);
                    return;
                }
            }
#endif

            Graphics.Blit(_vlcTexture, OutputTexture, flip, Vector2.zero);
        }

#if UNITY_ANDROID && !UNITY_EDITOR
        private Material GetAndroidColorConversionMaterial()
        {
            if (_androidColorConversionMaterial != null)
                return _androidColorConversionMaterial;

            var shader = Resources.Load<Shader>(AndroidColorConversionShaderResource);
            if (shader == null)
            {
                if (!_androidColorConversionShaderErrorLogged)
                {
                    Debug.LogError($"VLC Android color conversion shader was not found at Resources/{AndroidColorConversionShaderResource}.");
                    _androidColorConversionShaderErrorLogged = true;
                }

                return null;
            }

            _androidColorConversionMaterial = new Material(shader)
            {
        ...
        private string[] GetEffectiveMediaOptions(string path, string[] options)
        {
            var configuredOptions = options?.Length > 0 ? options : mediaOptions;
            var finalOptions = new List<string>(configuredOptions ?? Array.Empty<string>());

#if UNITY_ANDROID && !UNITY_EDITOR
            if (forceRtspOverTcpOnAndroid &&
                Uri.TryCreate(path?.Trim('"'), UriKind.Absolute, out Uri uri) &&
                string.Equals(uri.Scheme, "rtsp", StringComparison.OrdinalIgnoreCase) &&
                !finalOptions.Any(option =>
                    string.Equals(option?.TrimStart(':'), "rtsp-tcp", StringComparison.OrdinalIgnoreCase) ||
                    string.Equals(option?.TrimStart(':'), "rtsp-udp", StringComparison.OrdinalIgnoreCase)))
            {
                finalOptions.Add(":rtsp-tcp");
                Log("VLCMediaPlayer Android RTSP transport: TCP");
            }
#endif

            return finalOptions.ToArray();
        }
...
#if UNITY_ANDROID && !UNITY_EDITOR
            // LibVLCSharp 4.0's MediaParser callback delegates are not annotated with
            // MonoPInvokeCallback, so Unity IL2CPP cannot marshal them on Android.
            // Playback itself does not require pre-parsing; LibVLC discovers tracks
            // and metadata while opening the media.
            Log("Skipping media pre-parse on Android IL2CPP.");
#else
